...
# --------------------------------------------------
# PROCESO ETL: VISACIÓN COMPRAS POR EE
# --------------------------------------------------
def cargar_visacion_compras_ee(conexion, ruta_archivo):
    ...
    try:
        ...
        # Forzamos la limpieza del nombre de las columnas (minúsculas y sin espacios)
        df.columns = df.columns.str.strip().str.lower()
        
        # Rellenamos los nulos generales de campos de texto
        df = df.fillna("Sin información")
        
        # Iniciamos bloque transaccional seguro
        conexion.start_transaction()
        
        # Limpieza previa de la tabla antes de sobreescribir
        ...
        for index, row in df.iterrows():
            # Mapeo exacto de las 49 columnas del archivo Excel
            valores = (
        ...
        # 🚀 Ejecución masiva ultrarrápida en un único viaje a la BD
        if lista_lote:
            cursor.executemany(sql, lista_lote)
        
        conexion.commit()
        print(f"✅ Éxito: Se han cargado {len(lista_lote)} registros de Visación EE perfectamente.")

    except mysql.connector.Error as e:
        conexion.rollback()
        print(f"❌ Error de MySQL detectado: {e} | 🔄 Rollback completado.")
    except Exception as e:
        conexion.rollback()
        print(f"❌ Error General en el script Python: {e} | 🔄 Rollback completado.")
    finally:
        cursor.close()
